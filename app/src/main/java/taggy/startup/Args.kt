...
class Args(private val options: CommandLine) {
    ...
    val postfix by Companion.postfix.withDefaultNull()
    val postfixSeparator by Companion.postfixSeparator.withDefault("-")

    val version by Companion.version.withDefaultNull()

    val push by Companion.push.withFlag()
    val force by Companion.force.withFlag()
    val everywhere by Companion.everywhere.withFlag()

    val pushTarget by Companion.pushTarget.withDefaultNull()

    val pattern by Companion.pattern.withRegex()
